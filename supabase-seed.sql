...
-- Generates ~50 unused coupon codes per coupon type (33 types)
-- ============================================================
...
BEGIN
  ...
  -- (prefix, type, country, discount_value, discount_type)
  FOR rec IN (
    SELECT * FROM (VALUES
      -- ── Malaysia ─────────────────────────────────────────
    ...
    ) AS t(prefix, ctype, country, dval, dtype)
  ) LOOP

    -- Generate 50 codes per type
    FOR i IN 1..50 LOOP
      -- Build a 6-character random alphanumeric suffix (no ambiguous chars)
      sfx := '';
      FOR j IN 1..6 LOOP
      ...
      VALUES
        (cd, rec.ctype, rec.country, rec.dval, rec.dtype, FALSE, NULL)
